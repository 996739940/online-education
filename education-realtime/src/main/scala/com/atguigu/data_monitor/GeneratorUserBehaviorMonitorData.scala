...
object GeneratorUserBehaviorMonitorData {
  ...
  def main(args: Array[String]): Unit = {
    if(args.length != 1){
      println("Usage:Please input data like 2019-04-02")
      System.exit(1)
    }
  ...
  def generatorMonitorData(date: String): Unit = {
    ...
    //初始化时间戳，精确到秒 jdk1.8新日期api
    val formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd")
    val eventTime = LocalDate.parse(date, formatter)
    val initTimestamp = LocalDateTime.of(eventTime,LocalTime.MIN).atZone(ZoneId.of("+8")).toInstant().toEpochMilli() / 1000;
    ...
    println(eventTime + "=====" + initTimestamp)
  }
